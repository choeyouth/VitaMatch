...
public class ChatGptController {
	...
	@Value("${api-key.chatGpt.key}")
    private String key;

	// PostMapping으로 변경
    @PostMapping("/api/send")
    public ResponseEntity<String> send(@RequestBody UserMessage userMessage) {
        ...
        list.add(new Message("user", userMessage.getMessage()));

        // Body에 모델과 메시지 리스트 설정
        ...
        try {
            ...
            JsonNode rootNode = objectMapper.readTree(exchange.getBody());  // JSON 문자열을 파싱

            // "choices" 배열에서 첫 번째 항목의 "message.content" 값을 추출
            String content = rootNode.path("choices").get(0).path("message").path("content").asText();
            //System.out.println(">>>>>"+content);  // 출력

            return ResponseEntity.ok(content);  // 클라이언트에게 content만 응답
        } catch (Exception e) {
            e.printStackTrace();
